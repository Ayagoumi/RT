...
/*   Updated: 2021/03/26 18:24:37 by ayagoumi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void cal_end_cap(t_object *cyl, t_cap *ca, t_ray *r)
{
	...
	ca->m0 = dot(r->direction, cyl->orientation) * cyl->inter.t1 + dot(cyl->inter.oc, cyl->orientation);
	ca->m1 = dot(r->direction, cyl->orientation) * cyl->inter.t2 + dot(cyl->inter.oc, cyl->orientation);
}
...
double hit_cylinder(t_object *cyl, t_ray *r)
{
	...
	cyl->inter.a = dot(r->direction, r->direction) - pow(dot(r->direction, cyl->orientation), 2.0);
	cyl->inter.b = 2.0 * (dot(r->direction, cyl->inter.oc) - (dot(r->direction, cyl->orientation) * dot(cyl->inter.oc, cyl->orientation)));
